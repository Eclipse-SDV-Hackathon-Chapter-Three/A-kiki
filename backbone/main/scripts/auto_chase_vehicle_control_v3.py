...
class AutoChaseVehicleControl:
    ...
    def publish_collision_alarm(self, source, location, actor_id=None, details=None):
        ...
        try:
            ...
            alarm_payload = {
                ...
                'detected_at': now,
                'chase_active': bool(self.is_chasing),
                'details': details or {}
            }

            if self.zenoh_session:
                self.zenoh_session.put(self.collision_alarm_topic, json.dumps(alarm_payload))
                self.log_throttled('collision_alarm_publish',
                                   f"📡 Collision alarm ({source}) @ {location_dict}",
                                   interval=5.0,
                                   force=True)
            else:
                self.log_throttled('collision_alarm_failed',
                                   f"⚠️ Collision alarm not sent (Zenoh unavailable)",
                                   interval=5.0)
    ...
    def _to_location_dict(self, location):
        ...
        try:
            vehicle_loc = self.vehicle.get_location() if self.vehicle else carla.Location(0, 0, 0)
            return {'x': float(vehicle_loc.x), 'y': float(vehicle_loc.y), 'z': float(vehicle_loc.z)}
        except Exception:
