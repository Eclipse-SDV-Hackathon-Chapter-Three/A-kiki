...
class BoundingBoxDetector:
    ...
    def _get_actor_bounding_box(self, actor, world_2_camera, max_distance, actor_type):
        ...
        try:
            ...
            # 바운딩 박스가 화면 내에 있는지 확인 (수정된 조건)
            bbox_width = x_max - x_min
            bbox_height = y_max - y_min
            
            # 화면 경계에 대한 여유를 두고 체크 (더 관대한 조건)
            margin = 50  # 화면 경계에서 50픽셀 여유
            
            # 바운딩 박스 크기 제한 (완화된 필터링)
            max_bbox_width = self.image_w * 0.8   # 화면 너비의 80% 이하
            max_bbox_height = self.image_h * 0.8  # 화면 높이의 80% 이하
            min_bbox_width = 10   # 최소 10픽셀 (작은 바운딩 박스 허용)
            min_bbox_height = 10  # 최소 10픽셀 (작은 바운딩 박스 허용)
            
            # 화면 내에 있는지 확인 (완화된 조건)
            screen_margin = 50  # 화면 경계에서 50픽셀 여유
            in_screen = (x_max > -screen_margin and x_min < self.image_w + screen_margin and 
                        y_max > -screen_margin and y_min < self.image_h + screen_margin)
